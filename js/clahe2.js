...
(function(imageproc) {
    ...
    function clipHistogram(histogram, clipLimit) {
        ...
        // 关键：将余数“均匀地”散布到整个 0-255 范围
        // 而不是只加在前几个 bin
        var offset = Math.floor(256 / remainder);
        for (var i = 0; i < remainder; i++) {
            histogram[Math.min(i * offset, 255)]++;
        }
...
/**
     * Revised LUT Generation
     * Uses the correct normalization: (CDF(i) - minCDF) / (Total - minCDF) * 255
     */
    ...
    //     // If the tile is completely uniform (min == total), return identity
    ...
    //     for (var i = 0; i < 256; i++) {
    //         // This formula ensures the contrast is stretched 
    //         // relative to the available pixel distribution in the tile.
    ...
    function buildLutFromCdf(cdfInfo, totalPixels) {
        ...
        // OpenCV 的标准做法：直接使用 CDF 比例
        // 不需要特殊的 minCdf 减法，这样可以保证直方图被铺得更开
        var scale = 255.0 / totalPixels;

        for (var i = 0; i < 256; i++) {
            lut[i] = clampByte(cdf[i] * scale);
        }
    ...
    //     // Calculate the actual clip limit based on the tile area
    //     // Standard formula: (ClipLimit * TileSize^2) / 256
    //     var actualClipLimit = Math.max(1, Math.floor((clipLimit * totalPixels) / 256));
...
function buildTileLUT(tilePixels, clipLimit) {
    ...
    // 只有当 clipLimit > 1 时才进行裁剪，1 代表不增加对比度
